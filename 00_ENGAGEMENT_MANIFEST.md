...
The Engagement also prepares Parts 00–08 to serve as a controlled human-readable source for the later **Architecture-to-Delivery Layer**: requirements/specification derivation, independently acceptable end-to-end delivery increments, acceptance/traceability, a machine-readable Architecture Control Model, and deterministic/formal or other evidence-based architecture conformance verification where feasible. Detailed downstream tools are designed outside the architecture Parts; the Parts must preserve sufficient semantic precision for those tools to be derived without inventing new business meaning.
...
- Architecture-to-Delivery readiness of Parts 04–08, including downstream-verifiable architectural assertions without embedding future implementation/control technology into the Parts;
- controlled authoring methodology for Parts 04–08 through `runtime/ARCHITECTURE_PART_AUTHORING_STANDARD.md`.
...
canonical_paths:
  ...
  architecture_part_authoring_standard: runtime/ARCHITECTURE_PART_AUTHORING_STANDARD.md
  runtime_record: runtime/RUNTIME_DEPLOYMENT_RECORD.md
```
...
## 5A. Architecture Part Authoring Standard

`runtime/ARCHITECTURE_PART_AUTHORING_STANDARD.md`

Status: **ACTIVE CONTROLLED ENGAGEMENT METHOD — v1.1**

This standard governs authoring and completion-readiness of Parts 04–08. It requires the architecture Parts to remain human-readable and layer-appropriate while expressing material architectural rules precisely enough for later derivation into requirements, implementation specifications, acceptance criteria, traceability and architecture-conformance controls without inventing new business meaning.

The standard is **verification-aware but technology-neutral**. It may require an architectural assertion to be suitable for deterministic/formal/automated or evidence-based verification where appropriate, but it does not embed a future Architecture Control Model DSL, proof technology, AI mechanism or implementation design into the Parts.

It does not override authoritative Engagement business truth or governing ARCHITECT sources. It controls **how Parts 04–08 are authored and assessed for completion**, not the substantive business authority of the claims they contain.
...
### Cross-chat Learning Candidate Approval Trail

The learning-candidate trail is an **Engagement-wide control**, not a convention of one chat. Any runtime/chat working on this Engagement from the current controlled source set must:

- create or materially update a Learning Candidate when significant work produces transferable learning that justifies candidate treatment;
- record which Engagement artifacts were created or materially changed because of that learning, and why;
- record an explicit user disposition when the user approves, holds or rejects the candidate or associated method/change;
- preserve `NOT_REVIEWED` when no explicit disposition exists rather than inferring approval from silence, ordinary continuation or use of the output;
- verify the Learning Candidate trail at material Part handoff, phase transition or Engagement-close review so that later chats do not depend on conversational continuity to reconstruct learning history.

This requirement governs the **recording and continuity of Engagement learning**. It does not make Learning Candidates authoritative for NURA business truth.
...
- `ARCHITECTURE_PART_AUTHORING_STANDARD.md` — controlled method for authoring and completion-readiness of Parts 04–08; it does not override substantive Engagement authority;
